...
class MessageTrimmingGuardrail(CustomGuardrail):
    def __init__(
        ...
        **kwargs,
        ...
        default_config = self._get_default_config(self.optional_params.get('guardrail_name'))
    ...
    async def async_pre_call_hook(
        ...
        if "messages" in data and data["messages"]:
            ...
            available_for_completion = (
                max_context_tokens - int(current_tokens) - self.safety_buffer
            )
            safe_completion_tokens = min(
                requested_completion,
                max(
                    256, int(available_for_completion * 0.75)
                ),  # Use 75% to add extra margin
            )
            ...
            self._log_debug(
                f"After update - max_tokens: {data.get('max_tokens')}, max_completion_tokens: {data.get('max_completion_tokens')}"
            )

            # Now calculate max input tokens based on the safe completion tokens
            # Be even more conservative here
            max_input_tokens = int(
                (max_context_tokens - safe_completion_tokens - self.safety_buffer) * 0.85
            ...
            if current_tokens > max_input_tokens:
                ...
                try:
                    ...
                    available_after_trim = (
                            max_context_tokens - int(new_token_count) - self.safety_buffer
                    )
                    safe_completion_tokens = min(
                        requested_completion,
                        max(256, int(available_after_trim * 0.75))
                    )
                    self._log_debug(f"Recalculated safe completion tokens after trim: {safe_completion_tokens}")

                    # Update the data with recalculated value
                    if has_max_tokens:
                        data["max_tokens"] = safe_completion_tokens
                    elif has_max_completion:
                        data["max_completion_tokens"] = safe_completion_tokens
                    ...
                    current_tokens = new_token_count  # Update for final logging
                except Exception as e:
